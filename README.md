...
<p>roleStates	– таблица связки ролей с состояниями с колонками state (номер состояния) и role (номер роли). К точкам сборки и финальному состоянию роли не привязываются. Tсли из состояния есть переход в несколько состояний, то этим состояниям соответствуют разные роли;
